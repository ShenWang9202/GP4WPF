...
if(TestCase == 40)
    inpname='FCV_single.inp';
    %inpname='Example_Network_1_1.inp';
    %inpname='Example_Network_1.inp';
        acc = 0;
end
...
for i = 1:1
    ...
    X0(IndexInVar.PumpFlowIndex) = 10000;%InitialParameter.PipeFLowAverage(IndexInVar.PumpFlowIndex)*LPS2GMP;
    X0(IndexInVar.PipeFlowIndex) = InitialParameter.PipeFLowAverage(IndexInVar.PipeFlowIndex)*LPS2GMP;%(-1+rand(1,1)*(2));
...
KP = [];
for i = 1:index-2
    cellb = ((cellA{1,i+1})^(-1)*(cellA{1,i}));
    DeltaA{1,i} = (cellA{1,i+1})-(cellA{1,i});
    norm(DeltaA{1,i})*norm((cellA{1,i}),2);
    EigV = [EigV eig(cellAInverse{1,i})];
    Kp = norm((cellA{1,i}),2)*norm(cellAInverse{1,i},2);
    KP = [KP Kp];
    cellB{1,i} = cellb;
end
...
Solution_LPS = Wsolution;
i=1;
Solution_LPS(IndexInVar.JunctionHeadIndex,:) = Solution_LPS(IndexInVar.JunctionHeadIndex,:)/M2FT;
Solution_LPS(IndexInVar.ReservoirHeadIndex,:) = Solution_LPS(IndexInVar.ReservoirHeadIndex,:)/M2FT;
Solution_LPS(IndexInVar.TankHeadIndex,:) = Solution_LPS(IndexInVar.TankHeadIndex,:)/M2FT;
Solution_LPS(IndexInVar.PumpSpeedIndex,:) = Solution_LPS(IndexInVar.PumpSpeedIndex,:);
Solution_LPS(IndexInVar.PumpFlowIndex,:) = Solution_LPS(IndexInVar.PumpFlowIndex,:)/LPS2GMP;
Solution_LPS(IndexInVar.PipeFlowIndex,:) =  Solution_LPS(IndexInVar.PipeFlowIndex,:)/LPS2GMP;
Solution_LPS(IndexInVar.FCVValveFlowIndex,:) =  Solution_LPS(IndexInVar.FCVValveFlowIndex,:)/LPS2GMP;
ErrorWithEpanet = [];
[~,n] = size(Wsolution)
for i= 1:n
